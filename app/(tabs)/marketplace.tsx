...
import { ALBUMS, fmtPrice, fmtStars, COND_LABEL } from '../src/data';
...
export default function Marketplace() {
  ...
  return (
    <View style={s.wrap}>
      ...
      {/* Resultados */}
      <View style={s.resHd}>
        <Text style={s.resCnt}>{list.length} {list.length === 1 ? 'item' : 'itens'}</Text>
          ...
          <TouchableOpacity style={s.item} onPress={() => router.push(`/product/${a.id}`)}>
            ...
            <View style={s.info}>
              <View style={s.badges}>
                ...
                <View style={[s.condBadge, { backgroundColor: CCOLOR[a.condition] + '22' }]}>
                  <Text style={[s.condText, { color: CCOLOR[a.condition] }]}>
                    {COND_LABEL[a.condition].split(' ').slice(1).join(' ') || COND_LABEL[a.condition]}
                  </Text>
                </View>
              ...
              <View style={s.ratRow}>
                <Text style={s.stars}>{fmtStars(a.rating)}</Text>
                <Text style={s.rat}>{a.rating} ({a.reviews.toLocaleString()})</Text>
              </View>
              <View style={s.foot}>
                <View>
                  {a.origPrice && <Text style={s.orig}>{fmtPrice(a.origPrice)}</Text>}
...
const s = StyleSheet.create({
  ...
  fmtWrap: { height:44, justifyContent:'center' },
  fmtRow: { paddingHorizontal:12, gap:8, alignItems:'center', flexDirection:'row' },
  fc: { paddingHorizontal:12, paddingVertical:5, borderRadius:R.xxl, backgroundColor:C.s2, borderWidth:1, borderColor:C.border2 },
  fcOn: { backgroundColor:C.gold, borderColor:C.gold },
  fcT: { color:C.text2, fontSize:12 },
  fcTOn: { color:C.black, fontWeight:'700' },
  ...
  sortText: { color:C.gold, fontSize:12, fontWeight:'500' },
  sep: { height:1, backgroundColor:C.border, marginHorizontal:16 },
  item: { flexDirection:'row', padding:12, gap:12, alignItems:'center' },
  cover: { width:68, height:68, borderRadius:R.md, backgroundColor:C.s2, flexShrink:0 },
  info: { flex:1 },
  badges: { flexDirection:'row', alignItems:'center', gap:5, marginBottom:4, flexWrap:'wrap' },
  ...
  title: { color:C.cream, fontSize:14, fontWeight:'600' },
  artist: { color:C.text2, fontSize:12, marginTop:2, marginBottom:3 },
  ratRow: { flexDirection:'row', alignItems:'center', gap:5, marginBottom:6 },
  stars: { color:C.gold, fontSize:11 },
  rat: { color:C.text3, fontSize:11 },
  foot: { flexDirection:'row', alignItems:'center', justifyContent:'space-between' },
  orig: { color:C.text3, fontSize:11, textDecorationLine:'line-through' },
  price: { color:C.cream, fontSize:15, fontWeight:'700' },
  cartBtn: { backgroundColor:C.gold, paddingHorizontal:10, paddingVertical:5, borderRadius:R.lg },
  cartBtnText: { color:C.black, fontSize:11, fontWeight:'700' },
});